...
if ( ! -d "$opath" ) then
    echo "#############################################"
    echo "$opath is not a directory and now created.   "
    echo "#############################################"
    mkdir -p $opath 
#   exit 1
endif
...
#         set source = "https\://www.ncei.noaa.gov/data/climate-forecast-system/access/reforecast/6-hourly-by-pressure-level-9-month-runs/" 
#         set source = "https\://www.ncei.noaa.gov/data/climate-forecast-system/access/operational-9-month-forecast/6-hourly-by-pressure/"
          set source = "https\://nomads.ncep.noaa.gov/pub/data/nccf/com/cfs/prod/cfs.{$yyyymmdd}/${UTC}/6hrly_grib_01/"
#--
...
#		 sed s:SOURCE:"$source":g < $PWD/get.CFS.Rx.sub.FRM.csh | \
		 sed s:SOURCE:"$source":g < $PWD/get.CFS.Rx.sub.ope.FRM.csh | \
		 sed s:FILE:"$file":g | \
		 sed s:OPATH:"$opath":g | \
		 sed s:PWD:"$PWD":g | \
		 sed s:YY:"$yy":g | \
...
#-        set source = "https\://www.ncei.noaa.gov/data/climate-forecast-system/access/operational-9-month-forecast/6-hourly-flux/"
          set source = "https\://nomads.ncep.noaa.gov/pub/data/nccf/com/cfs/prod/cfs.{$yyyymmdd}/${UTC}/6hrly_grib_01/"
...
#	     set getCFS = $PWD/get.CFS.Rx.sub.flx.${xday}.csh 
